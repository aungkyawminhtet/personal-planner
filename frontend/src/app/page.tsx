      ...
      <div className="w-full max-w-2xl bg-white/80 backdrop-blur-xl border border-zinc-200 shadow-2xl rounded-3xl p-6 sm:p-10 space-y-8 relative z-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            />
            {errors.description && <span className="text-xs text-red-550 font-semibold">{errors.description.message}</span>}
          </div>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-violet-600 hover:bg-violet-550 active:bg-violet-700 text-white font-bold py-4 px-6 rounded-xl transition-all disabled:bg-zinc-200 disabled:text-zinc-400 flex justify-center items-center gap-2 text-sm shadow-lg shadow-violet-600/15 cursor-pointer"
          >
            {loading ? (
              <>
